...
import torch
import os
from pain_model.model import mini_XCEPTION
from const import detection_model_path, emotion_model_path, emotion_labels
...
def pain_convert(img, src):
    # 初始化为没有疼痛，防止一张脸都没有
    emotion_text = emotion_labels[str(1)]
    input_size = (48, 48)
    # 转换为灰度图
    ...
    with torch.no_grad():
        for face_coordinates in faces:
            ...
            emotion_text = emotion_labels[str(emotion_label_arg)]
            # 取出对应的值
            ...
            # 把标签写在图片里面
    return src, emotion_text
    # cv2.imshow("Image Window",src)
    # cv2.waitKey(0)  # 等待用户按下任意键
    # cv2.destroyAllWindows()  # 关闭所有OpenCV窗口


def convert_img(img_path, convert_image_path):
    """
    统一入口，方便服务端调用
    """
    print(img_path, convert_image_path)
    cv_img = cv2.imread(img_path)
    img = cv2.cvtColor(cv_img, cv2.COLOR_BGR2GRAY)
    src, emotion_text = pain_convert(img, cv_img)
    cv2.imwrite(convert_image_path, src)
    return emotion_text
